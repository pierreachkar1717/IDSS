...
df = pd.read_csv('../../data/Processed/neighbourhood/2022_academic_level_neighbourhood.csv')

# Choose the column/columns in your data that you want to use for clustering
cols = ['elementary_highschool_bachelorsdegree', 'no_studies', 'primarystudies_schoolcertificate', 'university_studies', 'secondary_school']
#cols = ['elementary_highschool_bachelorsdegree']
...
#df = pd.read_csv('../../data/Processed/clustering/clustered_academic_level.csv')

# Here 0 means low, 1 means medium and 2 means high
#df['label'] = df['cluster'].map({0: 0, 1: 1})
...
#df.to_csv('../../data/Processed/clustering/labled/labled_academic_level.csv', index=False)
