...
df = pd.read_csv('../../data/Processed/neighbourhood/bus_stops_neighbourhood.csv')
...
#cluster_and_plot(df, cols, 4, 'bus_stops')
...
df = pd.read_csv('../../data/Processed/clustering/clustered_bus_stops.csv')

# Here 0 means low, 1 means medium and 2 means high
df['label'] = df['cluster'].map({0: 0, 1: 2, 2: 1, 3: 3})
...
df.to_csv('../../data/Processed/clustering/labled/labled_bus_stops.csv', index=False)
