...
df = pd.read_csv('../../data/Processed/neighbourhood/temporaray_exhibitions_neighbourhood.csv')

# Choose the column/columns in your data that you want to use for clustering
cols = ['temporaray_exhibitions']
...
#optimal_clusters_number(df, cols, 8) # 3 clusters

# Second: Call the cluster_and_plot function to cluster your data and plot the results
#cluster_and_plot(df, cols, 3, 'temporaray_exhibitions')
...
df = pd.read_csv('../../data/Processed/clustering/clustered_temporaray_exhibitions.csv')
...
df.to_csv('../../data/Processed/clustering/labled/labled_temporaray_exhibitions.csv', index=False)
