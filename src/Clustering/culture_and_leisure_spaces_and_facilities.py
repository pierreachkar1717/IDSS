...
df = pd.read_csv('../../data/Processed/neighbourhood/culture_leisure_spaces_neighbourhood.csv')

# Choose the column/columns in your data that you want to use for clustering
cols = ['culture_leisure_spaces']
...
#optimal_clusters_number(df, cols, 20) # 2/3 clusters

# Second: Call the cluster_and_plot function to cluster your data and plot the results
#cluster_and_plot(df, cols, 3, 'culture_leisure_spaces')
...
df.to_csv('../../data/Processed/clustering/labled/labeled_culture_leisure_spaces.csv', index=False)
